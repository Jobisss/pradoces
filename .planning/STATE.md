...
- (2026-08-04) **PRODUCTION MODE**: banco da VPS considerado live com dados reais a partir de agora — nenhuma alteração pode resetar/apagar o banco (regra completa em CLAUDE.md topo + PROJECT.md Constraints)
- (2026-08-04) **Entrega adicionada fora do roadmap original** (v1 era só retirada, RES-16 novo de fato): taxa fixa única pra cidade toda (`Configuracao.taxaEntregaPadrao`/`entregaAtiva`, desligada por padrão), entregador é o próprio filho (João Gabriel, carro próprio) — não modelado no sistema, é operação manual. Escolha de retirada/entrega no próprio formulário de reserva (`DeliveryMode.ENTREGA` novo, `Reserva.enderecoEntrega`/`taxaEntregaCongelada` — taxa congelada no ato, mesmo padrão de custo histórico imutável já usado em lote/receita). CHECK `reservas_entrega_endereco_xor` garante endereço+taxa só existem junto com ENTREGA. Escopo deliberadamente restrito a `tipo=PADRAO`; resgate continua só retirada. Migration em duas partes (`entrega_reserva` + `entrega_check_endereco`) porque Postgres não deixa usar um valor de enum novo (`ENTREGA`) na mesma transação em que ele foi criado — o CHECK que referencia `'ENTREGA'` foi pra uma migration separada, aplicada depois da primeira já commitada
