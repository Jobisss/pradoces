...
- **Global scope**: `E:/BRCONNECT/doces_mae/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/BRCONNECT/doces_mae/.claude/notes/`)
...
2. Glob `E:/BRCONNECT/doces_mae/.claude/notes/*.md` (if directory exists) — global notes
...
Global (E:/BRCONNECT/doces_mae/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/BRCONNECT/doces_mae/.claude/notes/` — works in any directory
